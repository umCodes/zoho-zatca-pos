...
async def create_walk_in_invoice(line_items, method: str = "Cash"):
    ...
    # Create invoice
    res = await zoho_client.request(path="/invoices", method="POST", json=format_invoice(invoice))
    data = res.json()
    ...
    # Mark as sent
    res = await zoho_client.request(path=f"/invoices/{invoice_id}/status/sent")
    ...
    res = await zoho_client.request(path="/customerpayments", method="POST", json=payment_payload)
